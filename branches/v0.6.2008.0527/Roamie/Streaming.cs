...
namespace Virtuoso.Miranda.Roamie
{
    public static partial class Streaming
    {
        ...
        public static void CopyStream(Stream source, Stream destination, ProgressCallback callback)
        {
            ...
            double percent = (double)source.Length / 100d, read = 0;
        ...
        public static void EncryptStream(Stream source, Stream destination, string key)
        {
            ...
            // CryptoStream::Dispose() closes the underlying stream, which is what we don't want, so we use an unclosable stream wrapper
            CustomizableStream permaDestination = new CustomizableStream(destination);

            ICryptoTransform transform = tripleDes.CreateEncryptor(keyBytes, ivBytes);            
        ...
        public static void DecryptStream(Stream source, Stream destination, string key)
        {
            ...
            // CryptoStream::Dispose() closes the underlying stream, which is what we don't want, so we use an unclosable stream wrapper
            CustomizableStream permaSource = new CustomizableStream(source);

            ICryptoTransform transform = tripleDes.CreateDecryptor(keyBytes, ivBytes);            
